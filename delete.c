...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "firstname", json_string((const char*)sqlite3_column_text(stmt, 1)));
            json_object_set_new(object, "lastname", json_string((const char*)sqlite3_column_text(stmt, 2)));
