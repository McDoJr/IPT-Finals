...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    dbres = sqlite3_bind_text(stmt, 2, info.fullname, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 3, info.email, -1, SQLITE_TRANSIENT); 
    dbres |= sqlite3_bind_text(stmt, 4, info.phone, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 5, info.age, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.gender, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 7, info.municipality, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 8, info.year, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 9, info.course, -1, SQLITE_TRANSIENT);
