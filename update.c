...
//gcc -o update.o update.c -lsqlite3 -ljansson  -lfcgi
...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "fullname", json_string(info.fullname));
    ...
    json_object_set_new(obj, "municipality", json_string(info.municipality));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id", "fullname", "email", "phone", "age", "gender", "municipality", "year", "course" };
    char** infovar[] = {&info->id,&info->fullname,&info->email,&info->phone,&info->age,&info->gender,&info->municipality,&info->year,&info->course};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   const char *sql_statement = "update students set fullname=?, email=?, phone=?, age=?, gender=?, municipality=?, year=?, course=? where id=?";;
   ...
   loadJSON(&info);
    ...
    // bitwise operator
    dbres = sqlite3_bind_text(stmt, 1, info.fullname, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 2, info.email, -1, SQLITE_TRANSIENT); 
    dbres |= sqlite3_bind_text(stmt, 3, info.phone, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 4, info.age, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 5, info.gender, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.municipality, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 7, info.year, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 8, info.course, -1, SQLITE_TRANSIENT);
    dbres = sqlite3_bind_text(stmt, 9, info.id, -1, SQLITE_TRANSIENT);
