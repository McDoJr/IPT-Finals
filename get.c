...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    rc = sqlite3_open("/var/www/ipt/bejerano/cgi-bin/db/data.db", &db);
    ...
    const char *sql = (id == 0) ? "SELECT * FROM students WHERE id > ?" : "SELECT * FROM students WHERE id = ?";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "fullname", json_string((const char*)sqlite3_column_text(stmt, 1)));
            json_object_set_new(object, "email", json_string((const char*)sqlite3_column_text(stmt, 2)));
            json_object_set_new(object, "phone", json_integer(sqlite3_column_int(stmt, 3)));
            json_object_set_new(object, "age", json_integer(sqlite3_column_int(stmt, 4)));
            json_object_set_new(object, "gender", json_string((const char*)sqlite3_column_text(stmt, 5)));
            json_object_set_new(object, "municipality", json_string((const char*)sqlite3_column_text(stmt, 6)));
            json_object_set_new(object, "year", json_integer(sqlite3_column_int(stmt, 7)));
            json_object_set_new(object, "course", json_string((const char*)sqlite3_column_text(stmt, 8)));
